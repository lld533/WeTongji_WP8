...
namespace WeTongji.Api.Util
{
    public static class RSAEncryptHelper
    {
        ...
        {
            using (var derStream = Application.GetResourceStream(new Uri("public_key.der", UriKind.RelativeOrAbsolute)).Stream)
            {
                var bytes = new Byte[derStream.Length];
                derStream.Read(bytes, 0, (int)derStream.Length);

                var param = new RSA.RSAParameters();


                X509Certificate cer = new X509Certificate(bytes);

                var RSAKeyInfo = X509PublicKeyParser.GetRSAPublicKeyParameters(cer.GetPublicKey());
                param.N = RSAKeyInfo.Modulus;
                param.E = RSAKeyInfo.Exponent;

                var crypto = new RSA.RSACrypto();
                crypto.ImportParameters(param);
                crypto.PaddingProvider = new RSA.PaddingProviders.PKCS1v1_5();

                var srcBytes = System.Text.UnicodeEncoding.UTF8.GetBytes(str);
                return Convert.ToBase64String(crypto.Encrypt(srcBytes));
            }
        }
    }
}
